...
from Subclasses import TrafficRecordDetailed
...
class View:
    ...
    def handle_option(self, option):
        ...
        if option == 1:
            ...
            format_choice = input("Enter 'B' for basic format or 'D' for detailed format: ").lower()
            if format_choice == 'd':
    ...
    def search_records(self):
        """
        Prompt the user to enter search criteria and display the matching records.
        """
        column_names = [
            'section_id', 'highway', 'section', 'section_length',
            'section_description', 'date', 'description', 'group',
            'type', 'county', 'pTrucks', 'adt', 'aadt',
            'direction', 'pct85', 'priority_points'
        ]

        print("You may search using the following columns:")
        for column in column_names:
            print(f"- {column}")

        search_criteria = {}
        while True:
            column = input("Enter column name to search (or 'done' to finish): ")
            ...
            if column in column_names:
                ...
                print("Invalid column name. Please choose from the listed column names.")

        results = self.model.search_records(search_criteria)
        if results:
            format_choice = input("Enter 'B' for basic format or 'D' for detailed format: ").lower()
            count = 0
            if format_choice == 'd':
                for record in results:
                    detailed_record = TrafficRecordDetailed(
                        record.section_id, record.highway, record.section, record.section_length, record.section_description,
                        record.date, record.description, record.group, record.type, record.county, record.pTrucks,
                        record.adt, record.aadt, record.direction, record.pct85, record.priority_points
                    )
                    detailed_record.display()
                    count += 1
                    if count % 10 == 0:
